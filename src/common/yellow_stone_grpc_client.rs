...
pub async fn grpc_subscribe(grpc_addr: String, sub_account: String) -> anyhow::Result<()> {
    info!("Starting GRPC client, subscribed account: {}", sub_account);
    ...
    let args = Args {
        ...
        commitment: Some(ArgsCommitment::Processed),
        action: Action::Subscribe(Box::from(ActionSubscribe {
            accounts: false,
            ...
            transactions_account_include: vec![sub_account.clone()],
...
pub static CHANNEL: Lazy<(watch::Sender<String>, watch::Receiver<String>)> =
    Lazy::new(|| watch::channel(String::default()));

fn print_update(kind: &str, created_at: SystemTime, filters: &[String], value: Value) {
    let unix_since = created_at
        .duration_since(UNIX_EPOCH)
        .expect("valid system time");
    let json_str = serde_json::to_string(&value).expect("json serialization failed");
    info!(
        "{kind} ({}) at {}.{:0>6}: grpc got, detail: {}",
        filters.join(","),
        unix_since.as_secs(),
        unix_since.subsec_micros(),
        json_str
    );
    CHANNEL.0.send(json_str).expect("failed to send value");
}
